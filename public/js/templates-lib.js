;( function( elementor, $, window ) {
	...
	var TemplateLibraryInsertTemplateBehavior = Marionette.Behavior.extend({
		...
		onInsertButtonClick: function onInsertButtonClick() {
			...
			$e.run('mpdl/insert-template', args);
		}
	});
	...
	var TemplateLibraryTemplateELPView = TemplateLibraryTemplateView.extend({
		...
		onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
			...
			mpdl_templates_lib.templates.markAsFavorite(this.model, isFavorite);

			if (!isFavorite && mpdl_templates_lib.templates.getFilter('favorite')) {
	...
	var TemplateLibraryHeaderPreviewView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-preview-mpdl',
	...
	var TemplateLibraryHeaderBackView = Marionette.ItemView.extend( {
		...
		onClick: function() {
			$e.routes.restoreState( 'mpdl' );
		},
	} );

	var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
		template: '#tmpl-elementor-template-library-templates-mpdl',
		...
		filter: function filter(childModel) {
			var filterTerms = mpdl_templates_lib.templates.getFilterTerms(),
				passingFilter = true;
			jQuery.each(filterTerms, function (filterTermName) {
				var filterValue = mpdl_templates_lib.templates.getFilter(filterTermName);
		...
		addSourceData: function addSourceData() {
			...
			this.$el.attr('data-template-source', isEmpty ? 'empty' : mpdl_templates_lib.templates.getFilter('source'));
		},
		...
		toggleFilterClass: function toggleFilterClass() {
			this.$el.toggleClass('elementor-templates-filter-active', !!(mpdl_templates_lib.templates.getFilter('text') || mpdl_templates_lib.templates.getFilter('favorite')));
		},
		onRender: function onRender() {
			if ('mpdl' === mpdl_templates_lib.templates.getFilter('source')) {
				this.setFiltersUI();
			}
		},
		...
		onTextFilterInput: function onTextFilterInput() {
			mpdl_templates_lib.templates.setFilter('text', this.ui.textFilter.val());
		},
		onSelectFilterChange: function onSelectFilterChange(event) {
			var $select = jQuery(event.currentTarget),
				filterName = $select.data('elementor-filter');
			mpdl_templates_lib.templates.setFilter(filterName, $select.val());
		},
		onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
			mpdl_templates_lib.templates.setFilter('favorite', this.ui.myFavoritesFilter[0].checked);
		},
	...
	var TemplateLibraryModalLogoView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-templates-modal__header__logo_mpdl',
		className: function className() {
	...
	var TemplateLibraryHeaderActionsView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-actions-mpdl',
		...
		onSyncClick: function() {
			...
			mpdl_templates_lib.templates.requestLibraryData({
				onUpdate: function onUpdate() {
					self.ui.sync.removeClass('eicon-animation-spin');
					$e.routes.refreshContainer('mpdl');
				},
				forceUpdate: true,
				forceSync: true
			});
	...
	var TemplateLibraryHeaderMenuView = Marionette.ItemView.extend({
		...
		templateHelpers: function templateHelpers() {
			return {
				tabs: $e.components.get('mpdl').getTabs()
			};
		}
	});
	...
	var TemplateLibraryManager = function TemplateLibraryManager() {
		...
		this.requestLibraryData = function (options) {
			...
			elementorCommon.ajax.addRequest('mpdl_get_library_data', ajaxOptions);
		};
	...
	mpdl_templates_lib.templates = new TemplateLibraryManager();
	mpdl_templates_lib.templates.init();
	...
	elementor.on('document:loaded', function() {
		...
		$templateLibBtn.addClass( 'elementor-add-section-area-button mpdl-add-template-button' );
		$templateLibBtn.attr( 'title', 'Add Mp Design Library Template' );
		$templateLibBtn.html( '<img src="' + mpdl_templates_lib.logoUrl + '" />' );
		...
		$previewContents.find('body').delegate('.elementor-editor-element-add', 'click', function(e) {
			setTimeout(function() {
				...
				if ( sectionInner.length > 0 ) {
					sectionInner.find( '.mpdl-add-template-button' ).remove();
